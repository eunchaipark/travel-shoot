...
public class StaySearchController {
    ...
    // 생성자에서 @Qualifier 사용
    public StaySearchController(@Qualifier("elasticsearchService") StaySearchService staySearchService) {
    ...
    public ResponseEntity<List<AutocompleteResponse>> autocomplete(
        ...
        log.info("🔍 지역 자동완성 요청 (ES) - keyword: {}", keyword);
    ...
    public ResponseEntity<SearchResponse> search(
            ...
            HttpSession session) {
        log.info("🔍 검색창 검색 요청 (ES) - region: {}, checkIn: {}, checkOut: {}, adults: {}, children: {}",
    ...
    public ResponseEntity<SearchResponse> filterSearch(
            ...
            @PageableDefault(size = 20) Pageable pageable) {
        log.info("🔍 필터 검색 요청 (ES) - 적용된 필터: {} 개", request.getActiveFilterCount());
    ...
    public ResponseEntity<SearchResponse> infiniteScroll(
            ...
            @PageableDefault(size = 20) Pageable pageable) {
        log.info("🔍 무한 스크롤 (ES) - page: {}, size: {}", pageable.getPageNumber(), pageable.getPageSize());
